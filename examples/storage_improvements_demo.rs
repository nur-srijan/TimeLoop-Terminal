use timeloop_terminal::{Storage, SessionManager, EventRecorder};
use std::thread;
...
fn test_backup_encryption() {
    ...
    plain_storage.export_session_to_file("plain-session", "plain_backup.json").unwrap();
    
    // Check if backup is plaintext (should contain JSON)
    let backup_content = std::fs::read_to_string("plain_backup.json").unwrap();
    println!("   Backup content preview: {}", &backup_content[..backup_content.len().min(100)]);
    ...
    encrypted_storage.export_session_to_file("encrypted-session", "encrypted_backup.json").unwrap();
    ...
    match import_storage.import_session_from_file("encrypted_backup.json") {
        ...
        Err(e) => {
            ...
            match plain_import_storage.import_session_from_file("encrypted_backup.json") {
                Ok(imported_id) => println!("   ✅ Plaintext backup imported successfully: {}", imported_id),
...
mod tests {
    ...
    fn test_backup_encryption_security() {
        ...
        encrypted_storage.export_session_to_file("security-test", "security_backup.json").unwrap();
